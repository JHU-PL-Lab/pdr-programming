(**
   This module defines an A-translation routine for OCaml ASTs.
*)
...
(** A context type for A-normalization. *)
type context = {
  mutable counter: int (* a fresh variable counter *)
};;

(** Creates a new A-normalization context. *)
let new_context () =
  { counter = 0 }
;;
...
(** A convenience wrapper for constructing identifier expressions. *)
let string_to_exp_ident s =
  let s_desc = Pexp_ident (locwrap (Lident s)) in
  {pexp_desc = s_desc; pexp_loc = !default_loc; pexp_attributes = []}
;;

(** A convenience wrapper for constructing variable patterns. *)
let string_to_pat_var s =
  let desc = Ppat_var (locwrap s) in
  {ppat_desc = desc; ppat_loc = !default_loc; ppat_attributes = []}
;;

(** An A-translation tool for OCaml ASTs.  The resulting AST will consist
    entirely of shallow expressions, using let-bindings where necessary. *)
(* FIXME: This tool does not yet process "when" clauses within patterns. *)
let rec a_translator e c =
  ...
  | Pexp_function l ->
    let guardmatch case =
      ...
      | None -> None
      | Some g -> Some (a_translator g c)
    in
    let casemap case =
      { pc_lhs = case.pc_lhs;
        pc_guard = guardmatch case;
        pc_rhs = a_translator case.pc_rhs c
      }
    in
    { pexp_desc = Pexp_function (List.map casemap l);
  ...
  | Pexp_fun (l, e_option, p, e1) ->
    ...
    { pexp_desc = Pexp_fun (l, new_e_option, p, new_e1);
      pexp_loc = !default_loc;
      pexp_attributes = []
    }
  | Pexp_apply (e1, l) ->
    ...
    let rec loop untranslated pairlist =
      ...
      | [] ->
        let exp_pairlist =
          List.map (fun (label, s) -> (label, (string_to_exp_ident s))) pairlist
        in
        { pexp_desc = Pexp_apply (string_to_exp_ident newexp_name,
                                  List.rev exp_pairlist);
          pexp_loc = !default_loc;
          pexp_attributes = []
        }
      | (lhl, lhe)::lt ->
    ...
    in
    [%expr
      let [%p string_to_pat_var newexp_name] = [%e newexp] in
      [%e loop l []]
    ]
  | Pexp_match (e0, l) ->
  ...
  | Pexp_try (e0, l) -> (*TODO: is this right? need to test*)
    ...
    let casemap case =
      ...
      | Some _ -> raise (Utils.Not_yet_implemented "Pexp_try with guard")
    in
    let newcaselist = List.map casemap l in
    { pexp_desc = Pexp_try (a_e0, newcaselist);
      pexp_loc = !default_loc;
      pexp_attributes = []
    }
  | Pexp_tuple l ->
  ...
  | Pexp_construct (construct, e_option) ->
    begin
      ...
      | Some e1 -> let s = new_var_name c in
        ...
        let c_exp =
          { pexp_desc = Pexp_construct (construct, Some v1_e);
            pexp_loc = !default_loc;
            pexp_attributes = []
          }
        in
  ...
  | Pexp_record (plist, e_option) ->
    let rec loop untranslated pairlist translated_e_option =
      ...
      | (l, e1)::lt ->
        ...
        [%expr
          let [%p a_e1_pat] = [%e a_e1] in
          [%e loop lt newpairlist translated_e_option]
        ]
    in
    begin
      match e_option with
      ...
      | Some e0 ->
        ...
        [%expr
          let [%p a_e0_pat] = [%e a_e0] in
          [%e loop plist [] (Some a_e0_exp)]
        ]
    end
  | Pexp_field (e1, l) ->
    let a_e1 = a_translator e1 c in
  ...
  | Pexp_ifthenelse (e1, e2, e_o) ->
    let a_e3 =
      match e_o with
      | None -> None
      | Some e3 -> Some (a_translator e3 c)
    in
    let a_e1 = a_translator e1 c in
    let a_e1_name = new_var_name c in
    let a_e1_pat = string_to_pat_var a_e1_name in
